...
import { MGW01_WAREHOUSE_ID, MGW01_LAYOUT_ROWS, MGW01_LAYOUT_SPOTS_PER_ROW } from '../constants';
...
import MGW01LayoutView from './MGW01LayoutView';
import TableCellsIcon from './icons/TableCellsIcon';

export const MGW01Page: React.FC = () => {
    ...
    const itemsInLocation = useMemo(() =>
        (finishedGoodsList || []).filter((item: FinishedGoodItem) => item.currentLocation === MGW01_WAREHOUSE_ID),
        [finishedGoodsList]
    );
    ...
    return (
        <div className="bg-white dark:bg-secondary-800 shadow-xl rounded-lg h-full flex flex-col">
            <header className="p-4 md:px-6 py-3 flex-shrink-0 border-b dark:border-secondary-700 flex flex-col sm:flex-row justify-between items-center gap-3">
                <h2 className="text-2xl font-semibold text-primary-700 dark:text-primary-300 whitespace-nowrap">
                    Magazyn Wyrobów Gotowych ({MGW01_WAREHOUSE_ID})
                </h2>
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    <div className="flex items-center bg-slate-200 dark:bg-secondary-700 p-1 rounded-lg">
                        <Button onClick={() => setViewMode('list')} variant={viewMode === 'list' ? 'primary' : 'secondary'} className="p-2 !shadow-none">
                            <TableCellsIcon className="h-5 w-5" />
                        </Button>
                        <Button onClick={() => setViewMode('layout')} variant={viewMode === 'layout' ? 'primary' : 'secondary'} className="p-2 !shadow-none">
            ...
            <div className="flex-grow overflow-auto scrollbar-hide">
                {sortedItems.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 dark:text-gray-400">
                        ...
                        <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300">Magazyn {MGW01_WAREHOUSE_ID} jest pusty</h3>
                        <p className="mt-1 text-sm max-w-sm">
                            {searchTerm ? `Nie znaleziono palet pasujących do: "${searchTerm}".` : "Brak palet w tej lokalizacji."}
                        </p>
                    </div>
                ) : (
                    viewMode === 'list' ? renderListView() : <MGW01LayoutView pallets={sortedItems} />
                )}
            </div>
        </div>
    );
