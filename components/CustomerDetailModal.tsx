...
const CustomerDetailModal: React.FC<CustomerDetailModalProps> = ({ isOpen, onClose, customer }) => {
    const { handleUpdateCustomer, handleLookupNip } = useWarehouseContext();
    ...
    const handleGusLookup = async () => {
        ...
        try {
            ...
            if (result.success && result.data) {
                ...
                setFeedback({ type: 'success', message: 'Dane pobrane pomyślnie.' });
            } else {
    ...
    const handleSave = () => {
        const result = handleUpdateCustomer(customer.value, formData);
        if (result.success) {
            setFeedback({ type: 'success', message: 'Zapisano dane klienta.' });
            ...
            <div className="bg-white dark:bg-secondary-800 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                <header className="px-6 py-4 border-b dark:border-secondary-700 bg-slate-50 dark:bg-secondary-900/50 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Karta Klienta</h2>
                    </div>
                ...
                <div className="flex-grow overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="p-4 bg-primary-50 dark:bg-primary-900/10 border border-primary-200 rounded-lg">
                        <h3 className="text-xs font-bold uppercase text-primary-700 mb-3 tracking-widest">Weryfikacja Kontrahenta</h3>
                ...
                <footer className="px-6 py-4 border-t dark:border-secondary-700 bg-slate-50 dark:bg-secondary-900/50 flex justify-end gap-3">
                    ...
                    <Button onClick={handleSave} variant="primary" className="px-8">Zapisz Klienta</Button>
                </footer>
            </div>
        </div>
    );
